-- Патч для добавления модуля рассылки в уже существующую БД
-- Выполните в SQL Editor Supabase, если БД была создана до добавления broadcast-модуля
...
CREATE INDEX IF NOT EXISTS idx_broadcasts_active ON public.broadcasts (status) WHERE status IN ('queued', 'sending');
CREATE INDEX IF NOT EXISTS idx_broadcast_recipients_pending ON public.broadcast_recipients (broadcast_id, status) WHERE status = 'pending';
